...
describe('Migration Dexie V4 vers V5', () => {
  ...
  it('préserve chaque table du véritable schéma V4, utilise les tables V5 et survit à une seconde ouverture', async () => {
    ...
    v5.close();
    const v6 = new SamtechCRMDatabase(name); await v6.open(); expect(v6.verno).toBeGreaterThanOrEqual(13);
    ...
    v6.close();
    const reopened = new SamtechCRMDatabase(name); await reopened.open(); expect(reopened.verno).toBeGreaterThanOrEqual(13);
