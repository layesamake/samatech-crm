...
export default function EditClientPage() {
  ...
  const { register, control, handleSubmit, reset, setValue, getValues, formState: { errors, isSubmitting } } = useForm<UpdateClientFormInput, unknown, UpdateClientInput>({
  ...
  return (
    <main className="mx-auto max-w-2xl space-y-5 p-4 pb-24 md:p-8">
      ...
      <form onSubmit={handleSubmit((data) => submit(data))} className="space-y-5 rounded-xl border bg-card p-5 text-card-foreground bg-background text-foreground">
        {error && <p role="alert" className="rounded-lg border border-red-500/20 bg-red-500/10 p-3 text-sm text-red-800 dark:text-red-200 bg-background text-foreground">{error}</p>}
        {warning && <div role="alert" className="space-y-2 rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 text-sm text-amber-800 dark:text-amber-200 bg-background text-foreground"><p>{warning}</p><Button type="button" variant="outline" onClick={handleSubmit((data) => submit(data, true))}>Confirmer quand même</Button></div>}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-1"><div className="flex items-center justify-between"><Label htmlFor="firstName">Prénom</Label><VoiceDictationButton fieldLabel="prénom" onTranscript={(value) => setValue('firstName', applyDictation(getValues('firstName') ?? '', value), { shouldDirty: true })} /></div><Input id="firstName" {...register('firstName')} /></div>
          <div className="space-y-1"><div className="flex items-center justify-between"><Label htmlFor="lastName">Nom</Label><VoiceDictationButton fieldLabel="nom" onTranscript={(value) => setValue('lastName', applyDictation(getValues('lastName') ?? '', value), { shouldDirty: true })} /></div><Input id="lastName" {...register('lastName')} /></div>
          <div className="space-y-1"><div className="flex items-center justify-between"><Label htmlFor="companyName">Entreprise</Label><VoiceDictationButton fieldLabel="entreprise" onTranscript={(value) => setValue('companyName', applyDictation(getValues('companyName') ?? '', value), { shouldDirty: true })} /></div><Input id="companyName" {...register('companyName')} /></div>
          <div className="space-y-1"><div className="flex items-center justify-between"><Label htmlFor="jobTitle">Fonction</Label><VoiceDictationButton fieldLabel="fonction" onTranscript={(value) => setValue('jobTitle', applyDictation(getValues('jobTitle') ?? '', value), { shouldDirty: true })} /></div><Input id="jobTitle" {...register('jobTitle')} /></div>
