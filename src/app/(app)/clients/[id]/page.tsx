...
export default function ClientDetailPage() {
  ...
  return <main className="mx-auto max-w-4xl space-y-5 p-4 md:p-8">
    ...
    <section className="rounded-xl border bg-card text-card-foreground p-5"><h2 className="font-semibold">Coordonnées et intérêts conservés</h2><dl className="mt-3 grid gap-2 text-sm sm:grid-cols-2"><div><dt className="text-muted-foreground">Téléphone</dt><dd>{client.contact.whatsappPhone}</dd></div><div><dt className="text-muted-foreground">Localité</dt><dd>{client.locationName || 'Non renseignée'}</dd></div><div><dt className="text-muted-foreground">Entreprise</dt><dd>{client.contact.companyName || 'Non renseignée'}</dd></div><div><dt className="text-muted-foreground">Produits demandés</dt><dd>{client.productNames.join(', ') || 'Aucun'}</dd></div><div><dt className="text-muted-foreground">Tags</dt><dd className="flex flex-wrap gap-1">{client.tags.length ? client.tags.map((tag) => <span key={tag.id} className="inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-medium" style={{ color: tag.color || '#0B6B2D', borderColor: tag.color || '#0B6B2D', backgroundColor: `${tag.color || '#0B6B2D'}20` }}><TagIcon icon={tag.icon} className="size-3" />{tag.name}</span>) : 'Aucun'}</dd></div></dl></section>
    <ContactTagsManager contactId={client.contact.id} initialTags={client.tags} />
    ...
    <section className="rounded-xl border bg-card text-card-foreground p-5"><h2 className="font-semibold">Notes commerciales</h2>{client.notes.length > 0 ? <ul className="mt-3 space-y-2">{client.notes.map((note) => <li key={note.id} className="whitespace-pre-wrap rounded-md bg-muted/50 p-3 text-sm">{note.content}</li>)}</ul> : <p className="mt-3 text-sm">Aucune note.</p>}</section>
    
    <section className="rounded-xl border bg-card text-card-foreground p-5">
      <OpportunityList contactId={client.contact.id} />
    </section>

    <section className="rounded-xl border bg-card text-card-foreground p-5"><div className="flex justify-between gap-3"><h2 className="font-semibold">Factures</h2><Link href={`/invoices/new?clientId=${client.profile.id}`} className="text-blue-800 dark:text-blue-200">Nouvelle facture</Link></div>{invoices.length === 0 ? <p className="mt-3 text-sm">Aucune facture.</p> : <ul className="mt-3 space-y-2">{invoices.map((item) => <li key={item.invoice.id}><Link href={`/invoices/${item.invoice.id}`} className="flex justify-between rounded-md border p-3"><span>{item.invoice.number || 'Brouillon'}</span><span>{item.invoice.status} · {formatMinor(item.invoice.grandTotalMinor, item.invoice.currency, item.invoice.currencyScale)}</span></Link></li>)}</ul>}</section>
    <section className="rounded-xl border bg-card text-card-foreground p-5"><div className="flex justify-between gap-3"><h2 className="font-semibold">Paiements</h2><Link href="/payments" className="text-blue-800 dark:text-blue-200">Tous les paiements</Link></div>{paymentItems.length === 0 ? <p className="mt-3 text-sm">Aucun paiement.</p> : <ul className="mt-3 space-y-2">{paymentItems.map(({ payment, invoiceNumber }) => <li key={payment.id} className={`rounded-md border p-3 ${payment.status === 'REVERSED' ? 'bg-muted text-muted-foreground' : ''}`}><div className="flex flex-wrap justify-between gap-2"><Link href={`/invoices/${payment.invoiceId}`} className="font-medium text-blue-800 dark:text-blue-200">{invoiceNumber}</Link><span>{payment.status}</span></div><p>{formatMinor(payment.amountMinor, payment.currency, payment.currencyScale)} · {PAYMENT_METHOD_LABELS[payment.method]} · {payment.paymentDate}</p>{payment.reversalReason && <p className="text-sm">Motif : {payment.reversalReason}</p>}</li>)}</ul>}</section>
    <section className="rounded-xl border bg-card text-card-foreground p-5"><h2 className="font-semibold">Chronologie commerciale</h2>{client.events.length === 0 ? <p className="mt-3 text-sm">Aucun événement.</p> : <ol className="mt-3 space-y-3">{client.events.map((event) => <li key={event.id} className="border-l-2 border-border pl-4"><div className="flex flex-wrap justify-between gap-2"><strong>{event.title}</strong><time className="text-xs">{new Date(event.occurredAt).toLocaleString('fr-FR')}</time></div>{event.summary && <p className="text-sm text-muted-foreground">{event.summary}</p>}<span className="text-xs text-muted-foreground">{event.type}</span></li>)}</ol>}</section>
  </main>;
}
