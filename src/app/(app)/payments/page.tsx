...
export default function PaymentsPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-5 p-4 md:p-8">
    ...
    <section className={`gap-2 sm:grid-cols-2 lg:grid-cols-4 ${showMobileFilters ? 'grid' : 'hidden md:grid'}`} aria-label="Filtres">
      <input aria-label="Rechercher un paiement ou une créance" className="h-11 rounded-md border px-3 bg-background text-foreground" placeholder="Facture, client, référence" value={query} onChange={(event) => setQuery(event.target.value)} />
      <input aria-label="Paiements ou échéances depuis" type="date" className="h-11 rounded-md border px-3 bg-background text-foreground" value={from} onChange={(event) => setFrom(event.target.value)} />
      <input aria-label="Paiements ou échéances avant" type="date" className="h-11 rounded-md border px-3 bg-background text-foreground" value={to} onChange={(event) => setTo(event.target.value)} />
      ...
      {view === 'PAYMENTS' ? <><select aria-label="Filtrer par mode" className="h-11 rounded-md border px-3 bg-background text-foreground" value={method} onChange={(event) => setMethod(event.target.value)}><option value="">Tous modes</option>{PAYMENT_METHODS.map((item) => <option key={item} value={item}>{PAYMENT_METHOD_LABELS[item]}</option>)}</select><select aria-label="Filtrer par statut de paiement" className="h-11 rounded-md border px-3 bg-background text-foreground" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Tous statuts</option>{PAYMENT_STATUSES.map((item) => <option key={item} value={item}>{item}</option>)}</select><select aria-label="Filtrer par facture" className="h-11 rounded-md border px-3 bg-background text-foreground" value={invoiceId} onChange={(event) => setInvoiceId(event.target.value)}><option value="">Toutes factures</option>{invoices.map(([id, number]) => <option key={id} value={id}>{number}</option>)}</select></> : <><select aria-label="Filtrer les créances par échéance" className="h-11 rounded-md border px-3 bg-background text-foreground" value={due} onChange={(event) => setDue(event.target.value)}><option value="">Toutes créances</option><option value="OVERDUE">Échues</option><option value="UPCOMING">Non échues</option></select><select aria-label="Trier les créances" className="h-11 rounded-md border px-3 bg-background text-foreground" value={sort} onChange={(event) => setSort(event.target.value as 'DUE_DATE' | 'BALANCE_DESC')}><option value="DUE_DATE">Échéance</option><option value="BALANCE_DESC">Solde décroissant</option></select></>}
    </section>
    {error && <p role="alert" className="rounded-md bg-red-500/10 p-3 text-red-800 dark:text-red-200">{error}</p>}
    {view === 'PAYMENTS' ? <section aria-label="Liste des paiements"><div className="mb-3 flex flex-wrap justify-between gap-2"><p role="status">{items.length} paiement(s)</p><strong>Total actif filtré : {formatMinor(filteredActive, currency, scale)}</strong></div>{loading ? <p>Chargement…</p> : items.length === 0 ? <p className="rounded-xl border border-dashed p-8 text-center bg-background text-foreground">Aucun paiement ne correspond aux critères.</p> : <div className="grid gap-3 md:grid-cols-2">{items.map(({ payment, invoiceNumber, clientName }) => <article key={payment.id} className={`rounded-xl border p-4 ${payment.status === 'REVERSED' ? 'bg-muted text-muted-foreground' : 'bg-card text-card-foreground'}`}><div className="flex justify-between gap-2"><Link className="font-semibold text-blue-800 dark:text-blue-200" href={`/invoices/${payment.invoiceId}`}>{invoiceNumber}</Link><span>{payment.status}</span></div><p>{clientName}</p><strong>{formatMinor(payment.amountMinor, payment.currency, payment.currencyScale)}</strong><p className="text-sm">{payment.paymentDate} · {PAYMENT_METHOD_LABELS[payment.method]}</p>{payment.reference && <p className="text-sm">Référence : {payment.reference}</p>}{payment.reversalReason && <p className="text-sm">Motif : {payment.reversalReason}</p>}</article>)}</div>}</section>
      : <section aria-label="Liste des créances"><p role="status" className="mb-3">{receivables.length} créance(s)</p>{receivables.length === 0 ? <p className="rounded-xl border border-dashed p-8 text-center bg-background text-foreground">Aucune créance ne correspond aux critères.</p> : <div className="grid gap-3 md:grid-cols-2">{receivables.map(({ invoice, clientName, daysOverdue }) => <article key={invoice.id} className="rounded-xl border bg-card text-card-foreground p-4 bg-background text-foreground"><div className="flex justify-between gap-2"><Link className="font-semibold text-blue-800 dark:text-blue-200" href={`/invoices/${invoice.id}`}>{invoice.number}</Link><span>{invoice.status}</span></div><p>{clientName}</p><p className="text-sm">Émission : {invoice.issueDate} · Échéance : {invoice.dueDate || 'Non renseignée'}</p>{daysOverdue > 0 && <p className="font-semibold text-red-800 dark:text-red-200">Échue depuis {daysOverdue} jour(s)</p>}<dl className="mt-2 grid grid-cols-3 gap-2 text-sm"><div><dt>Total</dt><dd>{formatMinor(invoice.grandTotalMinor, invoice.currency, invoice.currencyScale)}</dd></div><div><dt>Payé</dt><dd>{formatMinor(invoice.paidTotalMinor, invoice.currency, invoice.currencyScale)}</dd></div><div><dt>Solde</dt><dd className="font-semibold">{formatMinor(invoice.balanceMinor, invoice.currency, invoice.currencyScale)}</dd></div></dl></article>)}</div>}</section>}
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5" aria-label="Indicateurs d’encaissement">
      <article className="rounded-xl border bg-card text-card-foreground p-4 bg-background text-foreground"><p className="text-xs text-muted-foreground">Total encaissé actif</p><strong>{formatMinor(indicators.activeCollectedMinor, currency, scale)}</strong></article>
      <article className="rounded-xl border bg-card text-card-foreground p-4 bg-background text-foreground"><p className="text-xs text-muted-foreground">Restant à encaisser</p><strong>{formatMinor(indicators.remainingMinor, currency, scale)}</strong></article>
      <article className="rounded-xl border bg-card text-card-foreground p-4 bg-background text-foreground"><p className="text-xs text-muted-foreground">Partiellement payées</p><strong>{indicators.partiallyPaidCount}</strong></article>
      <article className="rounded-xl border bg-card text-card-foreground p-4 bg-background text-foreground"><p className="text-xs text-muted-foreground">Factures payées</p><strong>{indicators.paidCount}</strong></article>
      <article className="rounded-xl border bg-card text-card-foreground p-4 bg-background text-foreground"><p className="text-xs text-muted-foreground">Créances échues</p><strong>{indicators.overdueReceivablesCount}</strong></article>
    </section>
  </main>;
}
