...
export default function PaymentsPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-5 p-4 md:p-8">
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5" aria-label="Indicateurs d’encaissement">
      <article className="rounded-xl border bg-white p-4"><p className="text-xs text-slate-500">Total encaissé actif</p><strong>{formatMinor(indicators.activeCollectedMinor, currency, scale)}</strong></article>
      <article className="rounded-xl border bg-white p-4"><p className="text-xs text-slate-500">Restant à encaisser</p><strong>{formatMinor(indicators.remainingMinor, currency, scale)}</strong></article>
      <article className="rounded-xl border bg-white p-4"><p className="text-xs text-slate-500">Partiellement payées</p><strong>{indicators.partiallyPaidCount}</strong></article>
      <article className="rounded-xl border bg-white p-4"><p className="text-xs text-slate-500">Factures payées</p><strong>{indicators.paidCount}</strong></article>
      <article className="rounded-xl border bg-white p-4"><p className="text-xs text-slate-500">Créances échues</p><strong>{indicators.overdueReceivablesCount}</strong></article>
    </section>
