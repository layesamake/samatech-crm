...
export default function FollowUpsPage() {
  ...
  const [query, setQuery] = useState(''); const [priority, setPriority] = useState(''); const [channel, setChannel] = useState('');
  const [sort, setSort] = useState<'DUE_AT' | 'PRIORITY'>('DUE_AT'); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => item.contactName.toLowerCase().includes(query.toLowerCase()) && (!priority || item.priority === priority) && (!channel || item.channel === channel)).sort((a, b) => sort === 'PRIORITY' ? weights[a.priority] - weights[b.priority] || a.dueAt.localeCompare(b.dueAt) : a.dueAt.localeCompare(b.dueAt));
  }, [items, query, priority, channel, sort]);
  return <main className="mx-auto max-w-5xl space-y-5 p-4 md:p-8"><header className="flex items-center justify-between"><div><h1 className="text-2xl font-bold">Relances</h1><p className="text-muted-foreground">Les retards sont recalculés à chaque ouverture.</p></div><Link href="/follow-ups/new" className="rounded-md bg-blue-600 px-4 py-3 text-white">Nouvelle</Link></header>
    <div className="flex gap-2 overflow-x-auto">{views.map(([value, label]) => <button key={value} onClick={() => { if (value !== view) { setLoading(true); setError(''); setView(value); } }} className={`min-h-11 whitespace-nowrap rounded-md px-4 ${view === value ? 'bg-slate-900 text-white' : 'bg-slate-100'}`}>{label}</button>)}</div>
    ...
    {error ? <p role="alert" className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-700">{error}</p> : loading ? <p role="status" className="rounded-xl border p-8 text-center">Chargement des relances...</p> : filtered.length === 0 ? <p className="rounded-xl border border-dashed p-8 text-center">Aucune relance dans cette vue.</p> : <div className="grid gap-3 md:grid-cols-2">{filtered.map((item) => <Link key={item.id} href={`/follow-ups/${item.id}`} className="rounded-xl border bg-card text-card-foreground p-4"><div className="flex justify-between"><h2 className="font-semibold">{item.contactName}</h2><span className="text-xs font-semibold">{item.priority}</span></div><p className="mt-2">{new Date(item.dueAt).toLocaleString('fr-FR', { timeZone: item.timezone })}</p><p className="text-sm text-muted-foreground">{item.channel} · {item.reason || 'Sans motif'} · {item.status}{view === 'OVERDUE' ? ' · En retard' : ''}</p></Link>)}</div>}
  </main>;
}
