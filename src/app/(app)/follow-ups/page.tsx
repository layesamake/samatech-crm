...
const FollowUpCard = memo(({ item, view }: { item: FollowUpRecord & { contactName: string }, view: string }) => (
  <Link href={`/follow-ups/${item.id}`} className="rounded-xl border bg-card text-card-foreground p-4 bg-background text-foreground">
...
export default function FollowUpsPage() {
  ...
  const actionButton = (
    <Link href="/follow-ups/new" className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition-colors">Nouvelle</Link>
  );

  return <main className="mx-auto max-w-5xl space-y-5 p-4 md:p-8"><header className="flex items-center justify-between"><div><h1 className="text-2xl font-bold hidden md:block">Relances</h1><p className="text-muted-foreground hidden md:block">Les retards sont recalculés à chaque ouverture.</p></div>{portalContainer ? createPortal(actionButton, portalContainer) : <div className="ml-auto">{actionButton}</div>}</header>
    ...
    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><input aria-label="Rechercher une relance" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Rechercher un contact" className="h-11 rounded-md border px-3 bg-background text-foreground"/><select aria-label="Filtrer par priorité" value={priority} onChange={(e) => setPriority(e.target.value)} className="h-11 rounded-md border px-3 bg-background text-foreground"><option value="">Toutes priorités</option>{FOLLOW_UP_PRIORITIES.map((value) => <option key={value}>{value}</option>)}</select><select aria-label="Filtrer par canal" value={channel} onChange={(e) => setChannel(e.target.value)} className="h-11 rounded-md border px-3 bg-background text-foreground"><option value="">Tous canaux</option>{FOLLOW_UP_CHANNELS.map((value) => <option key={value}>{value}</option>)}</select><select aria-label="Trier les relances" value={sort} onChange={(e) => setSort(e.target.value as 'DUE_AT' | 'PRIORITY')} className="h-11 rounded-md border px-3 bg-background text-foreground"><option value="DUE_AT">Échéance</option><option value="PRIORITY">Priorité</option></select></div>
    {error ? <p role="alert" className="rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-red-800 dark:text-red-200 bg-background text-foreground">{error}</p> : loading ? <p role="status" className="rounded-xl border p-8 text-center bg-background text-foreground">Chargement des relances...</p> : filtered.length === 0 ? <p className="rounded-xl border border-dashed p-8 text-center bg-background text-foreground">Aucune relance dans cette vue.</p> : <div className="grid gap-3 md:grid-cols-2">{paginatedFiltered.map((item) => <FollowUpCard key={item.id} item={item} view={view} />)}</div>}
