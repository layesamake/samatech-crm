...
export default function ConvertProspectPanel({ contactId, displayName }: { contactId: string; displayName: string }) {
  ...
  return <section className="space-y-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
    ...
    <label className="block text-sm">Date et heure de conversion<input aria-label="Date de conversion" type="datetime-local" value={convertedAt} onChange={(event) => setConvertedAt(event.target.value)} className="mt-1 h-11 w-full rounded-md border bg-white px-3" /></label>
    <p className="text-xs text-slate-600">La création d’une facture sera disponible au Sprint 5.</p>
    <div className="flex gap-2"><button disabled={pending} onClick={async () => { setPending(true); setError(''); try { const client = await convert.execute(contactId, { convertedAt: new Date(convertedAt).toISOString() }); router.push(`/clients/${client.profile.id}`); } catch (caught: unknown) { setError(caught instanceof Error ? caught.message : 'Conversion impossible'); setPending(false); } }} className="h-11 flex-1 rounded-md bg-emerald-700 text-white">{pending ? 'Conversion...' : 'Confirmer la conversion'}</button><button onClick={() => setOpen(false)} className="h-11 rounded-md border bg-white px-4">Fermer</button></div>
  </section>;
}
