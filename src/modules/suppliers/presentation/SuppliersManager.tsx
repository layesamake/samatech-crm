...
export default function SuppliersManager() {
  ...
  return <section className="space-y-4">{message && <p role="alert" className="rounded-lg bg-destructive/10 p-3 text-sm text-destructive">{message}</p>}
    ...
    <button type="button" aria-label="Ajouter un fournisseur ou bénéficiaire" onClick={() => { setForm(EMPTY); setEditingId(undefined); setFormOpen(true); }} className="fixed bottom-20 right-5 z-40 flex size-14 items-center justify-center rounded-full bg-primary text-primary-foreground shadow-lg shadow-primary/30 transition-transform hover:scale-105 active:scale-95 md:bottom-8 md:right-8"><Plus className="size-6" /></button>
