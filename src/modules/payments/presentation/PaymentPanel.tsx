  ...
  return <section className="space-y-4 rounded-xl border bg-card text-card-foreground p-5 bg-background text-foreground" aria-labelledby="payment-history-title">
    ...
    {(invoice.status === 'EMISE' || invoice.status === 'PARTIELLEMENT_PAYEE') && <form onSubmit={submit} className="space-y-3 rounded-lg bg-muted/50 p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-sm">Montant ({invoice.currency})<input required inputMode="decimal" aria-label="Montant du paiement" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={amount} onChange={(event) => setAmount(event.target.value)} /></label>
        <label className="text-sm">Date<input required type="date" aria-label="Date du paiement" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={paymentDate} onChange={(event) => setPaymentDate(event.target.value)} /></label>
        <label className="text-sm">Mode<select aria-label="Mode de paiement" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={method} onChange={(event) => setMethod(event.target.value as PaymentMethod)}>{PAYMENT_METHODS.map((item) => <option key={item} value={item}>{PAYMENT_METHOD_LABELS[item]}</option>)}</select></label>
        <label className="text-sm">Compte de trésorerie<select aria-label="Compte de trésorerie" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={accountId} onChange={(event) => setAccountId(event.target.value)}><option value="">Aucun compte affecté</option>{accounts.map(acc => <option key={acc.id} value={acc.id}>{acc.name}</option>)}</select></label>
        <label className="text-sm sm:col-span-2">Référence facultative<input aria-label="Référence du paiement" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={reference} onChange={(event) => setReference(event.target.value)} /></label>
      </div>
      <label className="block text-sm">Note {method === 'OTHER' ? '(obligatoire pour Autre)' : 'facultative'}<textarea required={method === 'OTHER'} aria-label="Note du paiement" className="mt-1 min-h-20 w-full rounded-md border p-3 bg-background text-foreground" value={note} onChange={(event) => setNote(event.target.value)} /></label>
      ...
      <div className="flex flex-wrap gap-2"><button type="button" className="h-11 rounded-md border bg-card text-card-foreground px-4 bg-background text-foreground" onClick={() => setAmount(minorToPaymentInput(invoice.balanceMinor, invoice.currencyScale))}>Régler le solde</button><button disabled={pending} className="h-11 rounded-md bg-emerald-700 px-4 text-white">{pending ? 'Enregistrement…' : 'Enregistrer le paiement'}</button></div>
    </form>}
