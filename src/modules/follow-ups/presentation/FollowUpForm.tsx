...
import { FOLLOW_UP_CHANNELS, FOLLOW_UP_CHANNEL_LABELS, FOLLOW_UP_PRIORITIES, FOLLOW_UP_PRIORITY_LABELS, FollowUpInput, FollowUpRecord } from '../domain/follow-up';
...
export default function FollowUpForm({ initial, contactId }: { initial?: FollowUpRecord; contactId?: string }) {
  ...
  return <form onSubmit={submit} className="mx-auto max-w-xl space-y-4 rounded-xl border bg-card text-card-foreground p-4 bg-background text-foreground">
    {error && <p role="alert" className="text-red-800 dark:text-red-200">{error}</p>}
    {warning && <div className="rounded-md border border-amber-500/20 bg-amber-500/10 p-3 bg-background text-foreground"><p>{warning === 'PAST_DUE' ? 'Cette échéance est passée. Confirmez l’enregistrement historique.' : 'Une relance proche existe déjà. Confirmez la création.'}</p>{similar && <p className="mt-1 text-sm">Relance similaire : {new Date(similar.dueAt).toLocaleString('fr-FR', { timeZone: similar.timezone })} · {similar.channel} · {similar.reason || 'Sans motif'}</p>}<button type="button" onClick={(event) => void submit(event, true)} className="mt-2 font-medium text-amber-800 dark:text-amber-200">Confirmer quand même</button></div>}
    <label className="block">Contact *<select name="contactId" value={form.contactId} disabled={Boolean(initial || contactId)} onChange={(e) => setForm({ ...form, contactId: e.target.value })} className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground"><option value="">Sélectionnez...</option>{contacts.map((contact) => <option key={contact.id} value={contact.id}>{contact.name}</option>)}</select></label>
    <label className="block">Date et heure *<input name="dueAt" type="datetime-local" value={form.dueAt} onChange={(e) => setForm({ ...form, dueAt: e.target.value })} className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" /></label>
    <div className="grid grid-cols-2 gap-3"><label>Canal<select name="channel" value={form.channel} onChange={(e) => setForm({ ...form, channel: e.target.value as FollowUpInput['channel'] })} className="mt-1 h-11 w-full rounded-md border bg-background text-foreground px-3">{FOLLOW_UP_CHANNELS.map((value) => <option value={value} key={value}>{FOLLOW_UP_CHANNEL_LABELS[value]}</option>)}</select></label><label>Priorité<select name="priority" value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value as FollowUpInput['priority'] })} className="mt-1 h-11 w-full rounded-md border bg-background text-foreground px-3">{FOLLOW_UP_PRIORITIES.map((value) => <option value={value} key={value}>{FOLLOW_UP_PRIORITY_LABELS[value]}</option>)}</select></label></div>
    <label className="block">Motif<textarea name="reason" value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} className="mt-1 w-full rounded-md border p-3 bg-background text-foreground" /></label>
    {form.channel === 'WHATSAPP' && <label className="block">Modèle<select name="messageTemplateId" value={form.messageTemplateId} onChange={(e) => setForm({ ...form, messageTemplateId: e.target.value })} className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground"><option value="">Sans modèle</option>{templates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></label>}
