...
export function calculateStatistics(data: StatisticsData, options: StatisticsOptions): StatisticsReport {
  ...
  for (const invoice of data.invoices) {
    if ((invoice as unknown as Record<string, unknown>).type === 'ESTIMATE') continue;
  ...
  for (const invoice of data.invoices) {
    if (invoice.archivedAt || !safeInteger(invoice.grandTotalMinor) || (invoice as unknown as Record<string, unknown>).type === 'ESTIMATE') continue; const contact = contactById.get(clientById.get(invoice.clientProfileId)?.contactId ?? ''); const locality = segment(localityAcc, localityKey(contact)); const source = segment(sourceAcc, sourceKey(contact));
  ...
  for (const invoice of data.invoices) { const date = recordDate(invoice.issueDate, period.timezone); if (!invoice.archivedAt && ELIGIBLE_INVOICE_STATUSES.has(invoice.status) && invoice.currency === primaryCurrency && invoice.currencyScale === primaryCurrencyScale && safeInteger(invoice.grandTotalMinor) && inPeriod(date, period) && (invoice as unknown as Record<string, unknown>).type !== 'ESTIMATE') pointByKey.get(bucket(date!))!.billedMinor += BigInt(invoice.grandTotalMinor); }
