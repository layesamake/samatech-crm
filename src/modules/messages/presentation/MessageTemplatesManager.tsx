...
export default function MessageTemplatesManager() {
  ...
  return <div className="grid gap-6 lg:grid-cols-[minmax(280px,360px)_1fr]">
    <form onSubmit={submit} className="space-y-4 rounded-xl border bg-card text-card-foreground p-4">
      ...
      <div className="rounded-md bg-muted/50 p-3"><strong>Aperçu texte</strong><p className="mt-2 whitespace-pre-wrap">{form.content || 'Votre message apparaîtra ici.'}</p><p className="mt-2 text-xs">Détectées : {extractVariables(form.content).join(', ') || 'aucune'}</p></div>
