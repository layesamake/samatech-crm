...
export async function generateInvoicePdf(value: InvoiceAggregate): Promise<Uint8Array> {
  ...
  text('ENTREPRISE', { font: bold, size: 11 }); text([value.invoice.companySnapshot.displayName, value.invoice.companySnapshot.address, value.invoice.companySnapshot.phone, value.invoice.companySnapshot.email].filter(Boolean).join(' - '), { gap: 8 });
  text('CLIENT', { font: bold, size: 11 }); text([value.invoice.clientSnapshot.displayName || value.clientName, value.invoice.clientSnapshot.address, value.invoice.clientSnapshot.phone, value.invoice.clientSnapshot.email].filter(Boolean).join(' - '), { gap: 12 });
  text('LIGNES', { font: bold, size: 11, gap: 3 });
