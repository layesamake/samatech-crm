...
export function safePdfFilename(number?: string, status?: string, _type?: string) { const base = number || (status === 'BROUILLON' ? 'brouillon' : 'facture'); return `facture-${base.toLocaleLowerCase('fr').replace(/[^a-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '') || 'document'}.pdf`; }
