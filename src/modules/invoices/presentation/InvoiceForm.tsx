...
export default function InvoiceForm({ invoiceId }: { invoiceId?: string }) {
  ...
  return <main className="mx-auto max-w-4xl space-y-5 p-4 md:p-8"><header><h1 className="text-2xl font-bold">{invoiceId ? 'Modifier le brouillon' : 'Nouvelle facture'}</h1><p className="text-sm text-muted-foreground">Le numéro définitif sera attribué uniquement à l’émission.</p></header>{error && <p role="alert" className="rounded-md bg-red-500/10 p-3 text-red-800 dark:text-red-200">{error}</p>}
    ...
    <section className="space-y-3"><div className="flex flex-wrap gap-2">
      ...
      {productMode === 'SELECT' && <button type="button" className="rounded-md border px-4" onClick={addProduct}>Ajouter le produit</button>}
      <button type="button" className="rounded-md border px-4" onClick={() => setLines((current) => [...current, { ...blankLine(), position: current.length }])}>Ajouter une ligne libre</button>
    </div>
    ...
    <button type="button" disabled={pending || !clientId} onClick={() => void save()} className="h-12 w-full rounded-md bg-blue-700 text-white">{pending ? 'Enregistrement...' : 'Enregistrer le brouillon'}</button></main>;
}
