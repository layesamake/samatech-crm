...
export default function InvoiceForm({ invoiceId }: { invoiceId?: string }) {
  ...
  return <main className="mx-auto max-w-4xl space-y-5 p-4 md:p-8"><header><h1 className="text-2xl font-bold">{invoiceId ? `Modifier le brouillon` : `Nouveau document`}</h1><p className="text-sm text-muted-foreground">Le numéro définitif sera attribué uniquement à l’émission.</p></header>{error && <p role="alert" className="rounded-md bg-red-500/10 p-3 text-red-800 dark:text-red-200">{error}</p>}
    <section className="grid gap-3 rounded-xl border bg-card text-card-foreground p-4 sm:grid-cols-4 bg-background text-foreground">
      <div className="flex flex-col gap-1">
        <label className="text-sm">Type de document</label>
        <select aria-label="Type de document" className="h-11 w-full rounded-md border px-3 bg-background text-foreground" value={docType} onChange={(e) => setDocType(e.target.value as 'INVOICE')}>
      ...
      <div className="flex flex-col gap-1">
        ...
        {clientMode === 'SELECT' ? (
          <select aria-label="Client" className="h-11 w-full rounded-md border px-3 bg-background text-foreground" value={clientId} onChange={(event) => {
          ...
          <div className="flex flex-col gap-2 rounded-md border p-2 bg-muted/30 bg-background text-foreground">
            <input aria-label="Nom du nouveau client" autoFocus placeholder="Nom / Entreprise" className="h-9 w-full rounded-md border px-2 text-sm bg-background text-foreground" value={newClientName} onChange={e => setNewClientName(e.target.value)} />
            <input aria-label="Numéro WhatsApp" placeholder="Numéro WhatsApp" className="h-9 w-full rounded-md border px-2 text-sm bg-background text-foreground" value={newClientPhone} onChange={e => setNewClientPhone(e.target.value)} />
      ...
      <label className="text-sm">Date d’émission<input aria-label="Date d’émission" type="date" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={issueDate} onChange={(event) => setIssueDate(event.target.value)}/></label>
      <label className="text-sm">Échéance<input aria-label="Date d’échéance" type="date" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={dueDate} onChange={(event) => setDueDate(event.target.value)}/></label>
    </section>
    <section className="space-y-3"><div className="flex flex-wrap gap-2">
      ...
      {lines.length === 0 ? <p className="rounded-xl border border-dashed p-6 text-center bg-background text-foreground">Le brouillon ne contient aucune ligne.</p> : lines.map((line, index) => <article key={line.id} className="space-y-3 rounded-xl border bg-card text-card-foreground p-4 bg-background text-foreground"><div className="flex justify-between"><strong>Ligne {index + 1}</strong><div className="flex gap-2"><button type="button" aria-label={`Monter ligne ${index + 1}`} className="min-h-11 border px-3 bg-background text-foreground" onClick={() => move(index, -1)}>↑</button><button type="button" aria-label={`Descendre ligne ${index + 1}`} className="min-h-11 border px-3 bg-background text-foreground" onClick={() => move(index, 1)}>↓</button><button type="button" className="min-h-11 border px-3 text-red-800 dark:text-red-200 bg-background text-foreground" onClick={() => { if (confirm('Retirer cette ligne du brouillon ?')) setLines((current) => current.filter((item) => item.id !== line.id)); }}>Retirer</button></div></div><input aria-label={`Désignation ligne ${index + 1}`} className="h-11 w-full rounded-md border px-3 bg-background text-foreground" value={line.designation} onChange={(event) => updateLine(line.id!, { designation: event.target.value })}/><textarea aria-label={`Description ligne ${index + 1}`} className="w-full rounded-md border px-3 py-2 bg-background text-foreground" value={line.description ?? ''} onChange={(event) => updateLine(line.id!, { description: event.target.value })}/><div className="grid gap-3 sm:grid-cols-3"><label className="text-sm">Quantité<input aria-label={`Quantité ligne ${index + 1}`} inputMode="decimal" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={line.quantityText} onChange={(event) => updateLine(line.id!, { quantityText: event.target.value })}/></label><label className="text-sm">Prix unitaire ({currency})<input aria-label={`Prix ligne ${index + 1}`} type="number" min="0" inputMode="numeric" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={line.unitPriceMinor} onChange={(event) => updateLine(line.id!, { unitPriceMinor: Number(event.target.value) })}/></label><label className="text-sm">Remise<select aria-label={`Type remise ligne ${index + 1}`} className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={line.discountType} onChange={(event) => updateLine(line.id!, { discountType: event.target.value as EditableLine['discountType'], discountValue: 0 })}><option value="NONE">Aucune</option><option value="PERCENT">Pourcentage</option><option value="AMOUNT">Montant</option></select></label></div>{line.discountType !== 'NONE' && <label className="block text-sm">{line.discountType === 'PERCENT' ? 'Pourcentage (points de base, 100 = 1 %)' : `Montant (${currency})`}<input aria-label={`Valeur remise ligne ${index + 1}`} type="number" min="0" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={line.discountValue} onChange={(event) => updateLine(line.id!, { discountValue: Number(event.target.value) })}/></label>}{taxesEnabled && <label className="block text-sm">Taxe (points de base)<input aria-label={`Taxe ligne ${index + 1}`} type="number" min="0" max="10000" className="mt-1 h-11 w-full rounded-md border px-3 bg-background text-foreground" value={line.taxRateBasisPoints} onChange={(event) => updateLine(line.id!, { taxRateBasisPoints: Number(event.target.value) })}/></label>}<p className="text-right font-semibold">Total ligne : {formatMinor(calculated.lines[index]?.lineTotalMinor ?? 0, currency, scale)}</p></article>)}</section>
    <section className="grid gap-3 rounded-xl border bg-muted/50 p-4 sm:grid-cols-2 bg-background text-foreground"><label>Notes<textarea aria-label="Notes facture" className="mt-1 w-full rounded-md border p-3 bg-background text-foreground" value={notes} onChange={(event) => setNotes(event.target.value)}/></label><label>Conditions<textarea aria-label="Conditions facture" className="mt-1 w-full rounded-md border p-3 bg-background text-foreground" value={terms} onChange={(event) => setTerms(event.target.value)}/></label><div className="sm:col-span-2 space-y-1 text-right"><p>Sous-total : {formatMinor(calculated.totals.subtotalMinor, currency, scale)}</p><p>Remises : {formatMinor(calculated.totals.discountTotalMinor, currency, scale)}</p><p>Taxes : {formatMinor(calculated.totals.taxTotalMinor, currency, scale)}</p><strong className="text-lg">Total : {formatMinor(calculated.totals.grandTotalMinor, currency, scale)}</strong></div></section>
