...
export function formatMinor(value: number, currency: string, scale: number): string { assertSafeInteger(value, 'Montant'); const digits = String(value).padStart(scale + 1, '0'); const integer = scale ? digits.slice(0, -scale) : digits; const fraction = scale ? `,${digits.slice(-scale)}` : ''; return `${integer.replace(/\B(?=(\d{3})+(?!\d))/g, ' ')}${fraction} ${currency}`; }
