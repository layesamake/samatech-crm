...
La version courante est Dexie V10. La migration V9 → V10 est strictement additive : elle crée uniquement `securitySettings` avec `&id, updatedAt`. Elle ne contient aucun `upgrade()`, ne transforme aucune valeur et ne touche pas aux vingt tables V9. `migration-v10.test.ts` ouvre un schéma V9 réel contenant une donnée dans chaque table, migre, écrit la sécurité, ferme puis rouvre la base.

Le format de sauvegarde accepté est `samtech-crm`, version `1`, avec un schéma source compris entre 1 et 10 et les vingt collections obligatoires suivantes : `settings`, `sequences`, `locations`, `categories`, `products`, `contacts`, `prospectProfiles`, `prospectInterests`, `clientProfiles`, `tags`, `contactTags`, `notes`, `timelineEvents`, `followUps`, `messageTemplates`, `invoices`, `invoiceLines`, `payments`, `campaigns`, `campaignRecipients`. Les index Dexie et `securitySettings` sont exclus.

L'enveloppe contient l'application source, la version Dexie, l'heure ISO, les totaux et un SHA-256 calculé par Web Crypto sur une sérialisation récursive aux clés triées, hors champ `integrity`. Le format 1 est le seul format restaurable démontré. La restauration valide tout en mémoire puis remplace les vingt tables dans une transaction Dexie unique; `securitySettings` demeure indépendant. Taille maximale : 25 Mio; volume maximal : 250 000 enregistrements.

`backup.lastExportedAt` est une entrée versionnée de `settings`, enregistrée seulement après génération, relecture et déclenchement du téléchargement. La table documentaire `appMetadata` n'est pas créée en V10; les métadonnées réellement requises restent dans `settings` afin d'éviter une table vide et une migration sans usage.

## État V1 bêta

La version applicative `1.0.0-beta.1` conserve Dexie V10 : 20 tables métier exportables et `securitySettings` séparée. Le jeu de charge Sprint 10 crée de manière déterministe 1 000 contacts/profils prospects, 200 clients, 100 produits, 500 relances, 600 notes et événements, 200 factures, 600 lignes, 180 paiements, 20 campagnes et 400 destinataires, sans migration ni persistance en production.
